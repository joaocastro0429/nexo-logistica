...
export function criarGestao(db: DatabaseSync) {
  ...
  function consultar(sessao: Sessao, tipo: string, id: string) {
    const tabela = recurso(tipo);
    permitir(sessao, tabela);
    const row = db.prepare(`SELECT ${campos[tabela]} FROM ${tabela} WHERE id = ? AND tenant_id = ?`).get(id, sessao.tenantId);
    if (!row) throw new NotFoundException('Registro não encontrado.');
    return { ...row };
  }
  function listar(sessao: Sessao, tipo: string, busca = '') {
    const tabela = recurso(tipo);
    permitir(sessao, tabela);
    return db.prepare(`SELECT ${campos[tabela]} FROM ${tabela} WHERE tenant_id = ? AND (nome LIKE ? OR email LIKE ?) ORDER BY nome`).all(sessao.tenantId, `%${busca.slice(0, 150)}%`, `%${busca.slice(0, 150)}%`);
  }
  ...
  function salvar(sessao: Sessao, tipo: string, body: unknown, id?: string) {
    const tabela = recurso(tipo);
    permitir(sessao, tabela, true);
    const values = validar(tabela, body, Boolean(id));
    db.exec('BEGIN IMMEDIATE');
    try {
      const atual = id ? consultar(sessao, tabela, id) : null;
      if (tabela === 'usuarios') {
        if (atual?.perfil === 'Administrador' && values.perfil !== 'Administrador') protegerAdministrador(sessao);
        if (db.prepare('SELECT id FROM usuarios WHERE email = ? AND id != ?').get(values.email!, id || '')) throw new ConflictException('E-mail indisponível.');
      }
      const registroId = id || randomUUID();
      const chaves = Object.keys(values);
      if (id) db.prepare(`UPDATE ${tabela} SET ${chaves.map(c => `${c} = ?`).join(', ')} WHERE id = ? AND tenant_id = ?`).run(...Object.values(values), id, sessao.tenantId);
      else db.prepare(`INSERT INTO ${tabela} (id, tenant_id, ${chaves.join(', ')}) VALUES (?, ?, ${chaves.map(() => '?').join(', ')})`).run(registroId, sessao.tenantId, ...Object.values(values));
      // Qualquer edição de usuário revoga sessões antigas, inclusive após mudar perfil/senha.
      if (tabela === 'usuarios' && id) db.prepare('DELETE FROM sessoes WHERE usuario_id = ?').run(id);
      const result = consultar(sessao, tabela, registroId);
      db.exec('COMMIT');
      return result;
    } catch (error) { db.exec('ROLLBACK'); throw error; }
  }
  function protegerAdministrador(sessao: Sessao) {
    const total = db.prepare("SELECT COUNT(*) AS total FROM usuarios WHERE tenant_id = ? AND perfil = 'Administrador'").get(sessao.tenantId)!;
    if (Number(total.total) <= 1) throw new ConflictException('A empresa precisa manter pelo menos um administrador.');
  }
  function remover(sessao: Sessao, tipo: string, id: string) {
    const tabela = recurso(tipo);
    permitir(sessao, tabela, true);
    db.exec('BEGIN IMMEDIATE');
    try {
      const atual = consultar(sessao, tabela, id);
      if (tabela === 'usuarios') {
        if (atual.perfil === 'Administrador') protegerAdministrador(sessao);
        db.prepare('DELETE FROM sessoes WHERE usuario_id = ?').run(id);
      }
      db.prepare(`DELETE FROM ${tabela} WHERE id = ? AND tenant_id = ?`).run(id, sessao.tenantId);
      db.exec('COMMIT');
      return { ok: true };
    } catch (error) { db.exec('ROLLBACK'); throw error; }
  }
  function simular(sessao: Sessao, body: unknown) {
    const dados = objeto(body);
    ...
    const transportadora = consultar(sessao, 'transportadoras', texto(dados, 'transportadoraId'));
    const clienteId = texto(dados, 'clienteId', 150, false);
    const cliente = clienteId ? consultar(sessao, 'clientes', clienteId) : null;
    ...
    db.prepare('INSERT INTO simulacoes VALUES (?, ?, ?, ?)').run(result.id, sessao.tenantId, result.criada_em, JSON.stringify(result));
    return result;
  }
  function historico(sessao: Sessao, id?: string) {
    if (id) {
      const row = db.prepare('SELECT dados FROM simulacoes WHERE id = ? AND tenant_id = ?').get(id, sessao.tenantId);
      if (!row) throw new NotFoundException('Simulação não encontrada.');
      return JSON.parse(String(row.dados));
    }
    return db.prepare('SELECT dados FROM simulacoes WHERE tenant_id = ? ORDER BY criada_em DESC, id').all(sessao.tenantId).map(row => JSON.parse(String(row.dados)));
  }
