...
export function criarAutenticacao(repository: AutenticacaoRepository, redis: Redis) {
  async function session(token: string | undefined): Promise<Sessao | null> {
    ...
    return repository.buscarSessao(usuarioId, version);
  }
  async function login(email: string, senha: string, perfil: string) {
    ...
    const [salt, hash] = (user ? String(user.senhaHash) : `${'0'.repeat(32)}:${'0'.repeat(128)}`).split(':');
    ...
    await redis.set(chaveSessao(token), JSON.stringify({ usuarioId: user.id, version: user.sessionVersion }), { EX: SESSION_SECONDS });
    return token;
  }
